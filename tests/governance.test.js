...
const { createWorkflow } = require('../server/governance/workflowCore');
const config = require('../server/governance/config');
...
test('deterministic assessment fails closed and keeps a human approval boundary', () => {
  ...
  const result = workflow.deterministicAssessment({ measurementUnit:'inch', roomWidth:144, roomLength:180, catalogVersion:'catalog-v7', assetLicenseStatus:'verified', policyVersion:'policy-v2' }, actor(config.assessmentRoles[0]));
  assert.equal(result.disposition, 'eligible_for_dimension_review');
  assert.equal(result.automatedDecision, false);
  assert.equal(result.requiresHumanReview, true);
  expectCode(() => workflow.deterministicAssessment({ measurementUnit:'inch', roomWidth:144, roomLength:180, catalogVersion:'catalog-v7', assetLicenseStatus:'verified', policyVersion:'policy-v2' }, actor('unauthorized_role')), 'FORBIDDEN');
});
...
test('dual-control, persistence, tenant-scope, and audit contracts are encoded', () => {
  ...
  const migration = fs.readFileSync(path.join(__dirname, '../server/migrations/001_governed_workflow.sql'), 'utf8');
  ...
  const router = fs.readFileSync(path.join(__dirname, '../server/governance/routerFactory.js'), 'utf8');
