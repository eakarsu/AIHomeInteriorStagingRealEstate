# Governed staging deliverable workflow

## Scope

The new endpoint is `/api/governed-staging-deliverables`. It implements editable design alternatives tied to verified dimensions, quantities, budget/catalog versions, render manifests, and qualified approval. The state sequence is:

`requirements_captured → site_verified → alternatives_ready → quantity_review → budget_review → render_review → approved → exported`
...
- Case identity, evidence, and events are protected from destructive updates/deletes by database constraints and triggers. The event history is readable at `GET /api/governed-staging-deliverables/cases/:id/history`.
- Deterministic triage validates measurement units and positive dimensions, catalog version, licensed-asset status, takeoff/budget/render/export digests. It always returns `automatedDecision: false` and `requiresHumanReview: true`.
- No CAD/BIM/GIS, MLS, licensed asset catalog, render farm, or permitting data is supplied. Qualified designers and contractors approve use.
